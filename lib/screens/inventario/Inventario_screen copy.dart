...
class _InventarioScreenState extends State<InventarioScreen> with TickerProviderStateMixin {
  ...
  final Map<int, List<Map<String, dynamic>>> tempMap = {};
  final Map<int, Map<String, dynamic>> infoProductos = {};

  for (var p in productos) {
    // Aseguramos que 'p' es un Map antes de acceder a sus llaves
    final id = (p['id_producto'] ?? p['id']) as int; 
    tempMap[id] = [];
    infoProductos[id] = p;
  }

  for (var v in variantes) {
    // IMPORTANTE: v['id_producto'] debe ser el ID entero, no el objeto completo
    final dynamic rawId = v['id_producto'];
    final int idProd = (rawId is Map) ? rawId['id_producto'] : rawId;
    
    if (tempMap.containsKey(idProd)) {
      tempMap[idProd]!.add(v);
    }
  }

  final Map<Map<String, dynamic>, List<Map<String, dynamic>>> finalGrouped = {};
  infoProductos.forEach((id, dataProducto) {
    finalGrouped[dataProducto] = tempMap[id] ?? [];
  });

  setState(() {
    _productosAgrupados = finalGrouped;
  });
}
  ...
  String _sku(Map<String, dynamic> r) => r['id_variante']?.toString() ?? '-';
        ...
        actions: [
          IconButton(icon: const Icon(Icons.add), onPressed: () {
            String? catId = (_currentView == InventarioView.productos && _tabController != null && _tabController!.index > 0)
              ? _categorias[_tabController!.index - 1]['id_categoria'].toString() : null;
